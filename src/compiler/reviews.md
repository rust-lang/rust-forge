...
- The team can find suitable reviewers who can aid the PR author to break up the large change into
  ...
  larger change.
- The team does not have the bandwidth, or team members are not ready or willing or able to
...
It is acceptable to leave comment indicating the intended scope of the delegation, such as 
...
- A feature was stabilised accidentally.
...
Please include a temporary regression test, if applicable, in a separate commit to check that the
regression is actually addressed by the revert commit. In a reland, this temporary regression test
can be adapted or removed following improved test coverage as suitable.
