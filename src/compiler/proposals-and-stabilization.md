...
* To be accepted, a major change proposal needs two things:
...
* **Implementation notes:** You don't have to talk about the implementation normally, but if there
  are any key things to note (i.e., it was very invasive to implement or could impose significant
  maintenance burden), you might note them here.
* **Precedent, links, and related material:** Have there been similar proposals on other
  compilers/linkers/tools, like `clang` or `lld`? Are there any relevant prior discussions?
* **Alternatives, concerns, and key decisions:** Were there any alernatives considered? If so, why
  did you pick this design? Are there any tradeoffs/drawbacks? Are any changes instantly stable?
  Is the intention for your change to eventually be fully or partially stabilised? Are there any
...
- Significant internal refactorings/changes
  ...
  - Describe your proposed refactorings in detail in an MCP - optionally scheduling a steering
    ...
    may second the proposal and then the implementation can be merged with an r+
...
- Adding a attribute with intent to later stabilize
  - Follow the language team's process and have the implementation PR reviewed by a member of the
    compiler team
  - It's possible a t-types or t-compiler FCP may also be warranted but this should be identified
    by any reviewers
...
- Proposing a new target
  ...
  - New targets should be assigned to the compiler team co-leads to check for any licensing
    concerns
  - If the target is very different from existing targets, then an seconded MCP can be requested
- Promoting a target
  - **Propose using:** PR
  - **Approve using:** r+ (compiler leads)
...
- Demoting/removing a target from tier one to tier two
  - **Propose using:** RFC
  - **Approve using:** FCP
  - Write an RFC describing why the target should be demoted/removed and once discussion has
    concluded, an FCP can be started to approve the demotion/removal.
- Demoting/removing a target from tier two to tier three
  - **Propose using:** MCP
  - **Approve using:** FCP
...
- Adding a unstable target feature
  - **Propose using:** PR
  - **Approve using:** r+
