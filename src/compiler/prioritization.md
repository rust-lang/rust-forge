...
## P-critical
A `P-critical` issue is a potentially blocker issue. These issues will be raised at the compiler
team's triage meeting on a weekly basis.
...
Finally, the meeting agenda can be generated. Clone and build [`triagebot`][triagebot] and set two
environment variables:

- `GITHUB_TOKEN` with a GitHub API token
- `GOOGLE_API_KEY` with access to the compiler team calendar
