...
1. Check if it's a regression and label it accordingly (`regression-*` labels)
  1. If the issue is a regression, is there a bisection available?
...
most urgent issues to work on, so that the compiler team can focus on what matters the most.
